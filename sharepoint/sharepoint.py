...
class SharePoint:
    def __init__(self, sharepoint_site: str, sharepoint_site_name: str, sharepoint_doc: str) -> None:
    ...
    def _auth(self) -> ClientContext:
        return ClientContext(self.sharepoint_site).with_credentials(
    ...
    def create_sharepoint_directory(self, dir_name: str) -> str | str:
        """
        ...
        if dir_name:
            ...
            if result:
                ...
                return f"{self.sharepoint_doc}/{dir_name}"
            return None
        return None

    def _get_files_list(self, folder_name: str) -> FileCollection:
        conn = self._auth()
    ...
    def download_latest_file(self, folder_name: str) -> Any:
    ...
    def upload_file(self, file_name: str, folder_name: str, content: bytes) -> Any:
        conn = self._auth()
    ...
    def get_list(self, list_name: str) -> Any:
        conn = self._auth()
        target_list = conn.web.lists.get_by_title(list_name)
        return target_list.items.get().execute_query()

    def get_file_properties_from_folder(self, folder_name: str) -> list[dict]:
        files_list = self._get_files_list(folder_name)
        properties_list = []
        for file in files_list:
            file_dict = {
